...
```js
function storyWriter(addWord,erase) {
  // Your code goes here
  var addWord=addWord.concat('');
  return addWord.replaceAll(erase,'');
}
...
```js
function addDesignation(title,l) {
  // your code goes here
   var prefix = title.substring(0, l);
...
function changeSalary(currentSalary) {
  // Your code goes here
  var raise=currentSalary + 500;
  var lower=currentSalary - 500;
  var current=currentSalary;
}
...
var setFirstName=alert('enter your first name')+ lastName;
var setLastName= fisrtName + alert('enter your last name');
}
